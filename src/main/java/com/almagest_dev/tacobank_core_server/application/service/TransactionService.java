...
public class TransactionService {
    ...
    @Transactional
    public List<TransactionResponseDto> getTransactionList(TransactionListRequestDto requestDto) {
        // 계좌 조회
        List<Account> accounts = accountRepository.findAllByAccountNum(requestDto.getAccountNum());
        ...
        if (fintechUseNum == null || fintechUseNum.isEmpty()) {
            throw new IllegalArgumentException("계좌의 금융 ID(fintech_use_num)가 존재하지 않습니다.");
        }
        ...
        // 응답 매핑
        return mapToTransactionResponseDto(responseDto.getResList());
    }
    ...
    private List<TransactionResponseDto> mapToTransactionResponseDto(List<TransactionDetailApiDto> transactions) {
